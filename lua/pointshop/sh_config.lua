...
PS.Config.CommunityName = "Server do Kenaj"
...
PS.Config.PointsName = 'Pontos' -- What are the points called?
...
PS.Config.CalculateBuyPrice = function(ply, item)
	...
	-- 25% off for the 'regular' group
	if ply:IsUserGroup('regular') or ply:IsUserGroup('admin') then return math.Round(item.Price * 0.75) end
	
	return item.Price
end

PS.Config.CalculateSellPrice = function(ply, item)
	return math.Round(item.Price * 0.50) -- 75% or 3/4 (rounded) of the original item price
end
